...
rsync -avz --delete ops/ "${SSH_TARGET}:${VPS_PATH}/ops/"
...
echo "Installing and verifying managed cron jobs..."
ssh "${SSH_TARGET}" "set -euo pipefail; chmod 700 \"${VPS_PATH}/ops/run-cron.sh\"; install -m 0644 \"${VPS_PATH}/ops/puretide.cron\" /etc/cron.d/puretide; ((crontab -l 2>/dev/null || true) | grep -vE 'puretide\.ca/api/cron/(daily-labels|afternoon-labels|shipping-automation)' || true) > \"${VPS_PATH}/.deploy/root-crontab\"; crontab \"${VPS_PATH}/.deploy/root-crontab\"; if systemctl list-unit-files crond.service --no-legend 2>/dev/null | grep -q crond; then systemctl enable --now crond; systemctl is-active --quiet crond; else systemctl enable --now cron; systemctl is-active --quiet cron; fi"
