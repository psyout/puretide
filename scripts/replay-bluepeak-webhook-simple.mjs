...
if (!ORDER_NUMBER) {
	console.error('Usage: node replay-bluepeak-webhook-simple.mjs <ORDER_NUMBER>');
	process.exit(1);
}

// Load env vars from .env manually (simple version)
import { readFileSync } from 'fs';
let WEBHOOK_SECRET = process.env.BLUEPEAK_WEBHOOK_SECRET;
if (!WEBHOOK_SECRET) {
	try {
		const envContent = readFileSync('.env', 'utf8');
		const match = envContent.match(/^BLUEPEAK_WEBHOOK_SECRET=(.+)$/m);
...
if (!WEBHOOK_SECRET) {
	console.error('Missing BLUEPEAK_WEBHOOK_SECRET');
	process.exit(1);
}
