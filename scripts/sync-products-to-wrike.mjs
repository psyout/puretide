#!/usr/bin/env node
/**
 * Sync products from Google Sheets to Wrike Products folder
 *
 * This script:
 * 1. Reads all products from Google Sheets
 * 2. Checks which products already have Wrike inventory tasks
 * 3. Creates Wrike tasks for new products
 * 4. Sets initial stock=0, cost=0, reorderPoint=5
 *
 * Usage:
...
async function createProductTask(product) {
	...
	if (process.env.WRIKE_STOCK_FIELD_ID) {
		customFields.push({ id: process.env.WRIKE_STOCK_FIELD_ID, value: '0' });
	}
	...
	const description = `<p><b>Product ID:</b> ${product.id}</p><p><b>Category:</b> ${product.category || 'N/A'}</p><p>Inventory managed via Wrike. Stock and cost can be updated in the dashboard.</p>`;
...
async function getSheetProducts() {
	// Import the products from the lib
	const { readSheetProducts } = await import('../lib/stockSheet.js');
	return await readSheetProducts();
}
...
async function run() {
	console.log('Wrike Products Sync\n');
