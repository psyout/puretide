...
export function validateEnv(): EnvSchema {
	...
	try {
		...
		if (validatedEnv.NODE_ENV === 'production') {
			...
			if (validatedEnv.ENABLE_EMAIL_NOTIFICATIONS && !validatedEnv.SMTP_HOST) {
				console.warn('SMTP_HOST not configured - email notifications will be disabled');
			}
		}
...
export function getDigipayConfig() {
	...
	return {
		...
		allowedIps: env.DIGIPAY_POSTBACK_ALLOWED_IP?.split(',').map(ip => ip.trim()).filter(Boolean) || ['185.240.29.227'],
	};
}
